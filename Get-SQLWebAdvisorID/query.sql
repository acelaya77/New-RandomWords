...
FROM (
    ...
    LEFT JOIN [ODS_HR].[dbo].[S85_PERSTAT] AS PSTAT WITH (NOLOCK) ON PSTAT.PERSTAT_HRP_ID = PERS.ID
    INNER JOIN [ODS_HR].[dbo].[S85_PERPOS] AS PPOS WITH (NOLOCK) ON PPOS.PERPOS_ID = PSTAT.PERSTAT_PRIMARY_PERPOS_ID
    LEFT JOIN [ODS_HR].[dbo].[S85_POSITION] AS POS WITH (NOLOCK) ON POS.POSITION_ID = PPOS.PERPOS_POSITION_ID
	LEFT JOIN [ODS_HR].[dbo].[S85_DEPTS] AS DEPTS WITH (NOLOCK) ON POS.POS_DEPT = DEPTS.DEPTS_ID
    JOIN [ODS_HR].[dbo].[S85_HRPER] AS HRPER WITH (NOLOCK) ON HRPER.HRPER_ID = PPOS.PERPOS_HRP_ID
--	LEFT JOIN [ODS_ST].[dbo].[S85_PERSON] AS PERS WITH (NOLOCK) ON PERS.ID = PSTAT.PERSTAT_HRP_ID
    ...
    LEFT JOIN ODS_ST.dbo.S85_PERSON_PIN AS PP WITH (NOLOCK) ON PP.PERSON_PIN_ID = PSTAT.PERSTAT_HRP_ID
